...
wich is the standard representation in most computers nowadays.
%
\begin{definition}[$\nattobe$ and $\betonat$]
...
This algorithm first derives a sequence of hashes (steps~\ref{step:nonce_construction:seed}
and~\ref{step:nonce_construction:first_hash}) then constructs a \emph{final} hash from
...
Finally, it shuffles the sequence (step~\ref{step:nonce_construction:swap}), which intuitively
guarantees that, in order to compute a given scoop of the nonce
(\textit{ie.\ }a pair of consecutive hashes), the whole algorithm must be executed.
%
