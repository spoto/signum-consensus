...
\begin{definition}[Trunk, Block]\label{def:trunk}
  ...
  \begin{align*}
    ...
    \tau,\alpha\in\mathbb{N},\ \alpha>0,\\
    \power,\weightedbeat\in\mathbb{N},\\
    \trunk\in\Trunks,\\
...
and $b.\alpha$ is its acceleration. If $b$ is a non-genesis block, then
$b.\power$ expresses how much space has been used to build the path to $b$,
starting from the genesis block; it will be used to select the \emph{best chain} for mining
$b$'s sons. The value of $b.\weightedbeat$ is the average block creation rate
in the path to $b$; it weighs the last blocks more. It will be
compared to $\beat$ (Tab.~\ref{tab:notations}) to understand if the acceleration
must be increased or decreased in $b$'s sons.
The value of $b.\previousblockhash$ is the hash of the previous block in the path to $b$.
...
\begin{definition}[Block's height]\label{def:block_height}
  Let $b\in\Blocks$. The \emph{height} of $b$, written $\height(b)$,
  is $0$ if $b\in\GenesisBlocks$, and $b.\trunk.\height$ if $b\in\NonGenesisBlocks$.  
%  \[
...
\begin{definition}[$\initialchallenge$]\label{def:initial_challenge}
  The \emph{initial challenge} is\footnote{In~\cite{SignumPlotting}
  the scoop number is derived from $\sigma_\genesis$; we simplify it to $0$, without loss
  of generality.}$\initialchallenge=\langle 0,\sigma_\genesis\rangle$,
  %
  %\[
  %  \initialchallenge=
...
as Bitcoin~\cite{WalkerG24,GarayKL17}, adapted to our context: the ratio between the maximal (hence worse)
...
\begin{definition}[Next block]\label{def:next_block}
  ...
  \nextweightedbeat(b,\delta),\langle\height(b)+1,\delta\rangle,h_\block(b)
...
\begin{definition}[Blockchain, Consensus]\label{def:blockchain}
  ...
  \item\label{prop:blockchain:consensus} for each $b\in B$, the predicate $\consensus(B,b)$ holds, where
    %
    \begin{itemize}
    \item if $b\in\GenesisBlocks$, then $\consensus(B,b)$ is just the consensus rule:
    ...
    \item if $b\in\NonGenesisBlocks$, then $\consensus(B,b)$ is the logical conjunction
      ...
      \item\label{prop:consensus:no_dangling} there are no dangling pointers:
        ...
        $ exists;%\]
      \item\label{prop:consensus:answer} $b$'s deadline answers the challenge of $p$ (Def.~\ref{def:next_challenge_from_block}):
        $%\[
        \nextchallenge(p)=b.\trunk.\delta.\challenge
        $%;\]
      \item\label{prop:consensus:next_block} $b$ is $p$'s next block \wrt $b$'s deadline (Def.~\ref{def:next_block}):
        $%\[
...
\begin{definition}[Blockchain network]\label{def:blockchain_network}
  ...
  Each peer holds a plot (Def.~\ref{def:plot}) on disk, starts with
...
it assumes that peers are fully connected, never disconnect and never need to synchronize.
In practice, peers do not hold plots but rely on (one or more)
external services (miners) that hold the plots.
...
Def.~\ref{def:blockchain_network} defines the
block mining algorithm. It is an infinite loop that
looks for the most powerful block $p$ in blockchain (step~\ref{step:block_mining:loop}),
derives a challenge $c$ from $p$ (step~\ref{step:block_mining:next_challenge}),
uses a plot to compute a best deadline $\delta$ for $c$ (step~\ref{step:block_mining:next_deadline}),
computes the next block $b'$ for $\delta$ (step~\ref{step:block_mining:next_block}) and waits for $\delta$
to expire (step~\ref{step:block_mining:wait}). Then it adds $b'$ in blockchain
(step~\ref{step:block_mining:add}), whispers $b'$ to all peers (step~\ref{step:block_mining:whisper})
and restarts.
%
\begin{alg}[Block mining]\label{alg:block_mining}
  ...
  \item\label{step:block_mining:whisper} whisper $b'$ to the peers connected to $P$;
  \item go back to step~\ref{step:block_mining:loop}.
  \end{enumerate}
\end{alg}
%
The block mined algorithm receives a block whispered from some connected peer
(step~\ref{step:block_mined:loop}), checks its validity
(step~\ref{step:block_mined:add}) and adds it to the blockchain.
%
\begin{alg}[Block mined]\label{alg:block_mined}
  ...
  %
  \begin{enumerate}
  \item\label{step:block_mined:loop} wait for a block $b$ whispered from some connected peer $P'$;
...
in order to avoid keeping useless blocks. This is not relevant here.
...
a rational peer would interrupt waiting at step~\ref{step:block_mining:wait}
of Alg.~\ref{alg:block_mining}, discard $b'$ and restart Alg.~\ref{alg:block_mining}
from step~1, since the whispered $b$ is better than the block $b'$ that it is being mined,
hence it is wiser to stop waiting and start mining on top of $b$.
These are optimizations and are not considered here.
...
The hypothesis on no hash collision is standard. Namely, all existing
blockchains make that hypothesis and they would all collapse otherwise,
because they all identify blocks by their hash.
%
