...
\begin{frame}{Blocks}
  ...
  \begin{center}
    ...
    \only<19>{\alert{Rule 7:} $\finalstateid=\previous.\finalstateid+\transactions$}
  \end{center}
...
\begin{frame}\frametitle{The Mokamint $\Leftrightarrow$ application API}
  ...
  \begin{algorithm}[H] 
    ...
    \begin{algorithmic}[1]
      ...
      \EndFor
      \State {$\finalstateid'$ $\gets$ {\color{red}endBlock}(groupId, $\nonce$)}\hspace*{8.5ex}\textbf{[git add]}
      \State {{\color{red}commitBlock}(groupId)}\hspace*{27ex}\textbf{[git commit]}
