...
\begin{frame}{The blockchain peer-to-peer network}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.5,>=Stealth]
      ...
      \only<2>{\filldraw [fill=gray!10!white] (0,8.25) circle [radius=1,very thick];}
  ...
  \begin{center}
    \only<1>{All peers hold a private copy of the same blockchain data structure.}
    \only<2>{Let us inspect a block of the blockchain.}
  \end{center}
...
\begin{frame}{Blocks (simplified)}
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      ...
      \only<1-14,16->{\draw [fill=Yellow, thick] (0,1.5) rectangle +(4.5,1);}
      \only<15>{\draw [inner color=Yellow,outer color=white, draw=black, thick] (0,1.5) rectangle +(4.5,1);}
      \visible<5->{\draw (2.25,2) node{$\tau$ (creation time)};}

      \only<1-6,8-15>{\draw [fill=SpringGreen, thick] (0,-1) rectangle +(4.5,2.5);}
      \only<7,16>{\draw [inner color=SpringGreen,outer color=white, draw=black, thick] (0,-1) rectangle +(4.5,2.5);}
      \visible<6-7>{\draw (2.25,0.3) node{transactions};}
      \visible<8->{\draw (2.25,1.1) node{Alice $\xRightarrow{100}$ Bob};}
      \only<16>{\draw (4,1.1) node{\includegraphics[width=0.5cm]{pictures/signature}};}
      \visible<9->{\draw (2.25,0.5) node{Fausto $\xRightarrow{500}$ John};}
      \visible<10->{\draw (2.25,-0.1) node{Donald $\xRightarrow{350}$ Alice};}
      \visible<11->{\draw (2.25,-0.7) node{Donald $\xRightarrow{2500}$ John};}
    \end{tikzpicture}
  \end{center}
  ...
  \begin{center}
    ...
    \only<15>{\alert{Rule 3:} The creation time is not in the future: $\tau\le\tau_\now$}
    \only<16>{\alert{Rule 4:} Each transaction is correctly signed by its sender}
  \end{center}
