...
\subtitle{S\~ao Lu\'{\i}s do Maranh\~ao, Brasil, November 2025}
...
\begin{frame}
  \frametitle{Outline}
  \begin{center}
    November 24, 2025, 2.30pm--4.30pm
  \end{center}
  \tableofcontents[part=1]
  \bigskip
  \begin{center}
    November 26, 2025, 2.30pm--5pm
  \end{center}
  \tableofcontents[part=2]
  \begin{center}
    November 28, 2025, 2.30pm--5.45pm
  \end{center}
...
\begin{frame}\frametitle{Proof of work}
  ...
  \begin{tabular}{ccc}
    ...
    $\begin{array}{c}
      ...
      \Rightarrow
    \end{array}$ &
    $\mathtt{\underbrace{\color{blue}{\mathtt{fffffff}}}_{\begin{array}{c}7\text{ digits}\\(difficulty)\end{array}}\hspace*{-2.1ex}43f5e6bc00d\ldots5974eda}$
  \end{tabular}
...
\begin{frame}\frametitle{Proof of space in detail}
...
\begin{frame}{Mokamint (Fausto Spoto, 2023--2025)}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.7]
      ...
      \draw (5,3.5) node [right] {e.g.\ Hotmoka, IDs, Prolog\ldots};
      \draw [decorate,decoration={brace,amplitude=4pt},thick,xshift=0.3cm,yshift=0pt]
...
\begin{verbatim}
docker run --rm -it mokamint/mokamint:1.6.3 /bin/bash
\end{verbatim}
  \end{greenbox}
...
\begin{verbatim}
mokamint-node keys create --name miner.pem
\end{verbatim}
  \end{greenbox}
...
\begin{frame}[fragile]{Become a miner of the Mokamint+Hotmoka testnet}
  ...
  \begin{center}
    \includegraphics[width=5cm]{pictures/Mokaminter}
  \end{center}

  \begin{center}
    $\approx 2\%$ of total CPU time\qquad $\approx 1.5$ megabytes exchanged per day
  \end{center}

\end{frame}

\begin{frame}[fragile]{Take part in our experiments}

  We are writing a scientific paper about ecological mining.

  Take part in the experiments with Mokaminter! We will thank you in the paper.

  \medskip

  \begin{enumerate}
  \item install Mokaminter in your Android phone
  \item turn notifications on for Mokaminter (from Settings, look for Mokaminter)
  \item create a miner in Mokaminter: it should be easy
  \item keep it running for a week
  \item open Mokaminter every morning: some Android devices kill it in the night
  \item after one week, tell us how much battery it consumed and how many bytes it transferred (from Settings, look for Mokaminter)
  \item write to \texttt{fausto.spoto@univr.it}
  \end{enumerate}
...
moka accounts create faucet 100000000000000
  your-key-here --uri ws://panarea.hotmoka.io
