...
\subtitle{WTSC'25, Miyakojima, Japan, 18/4/2025}
...
\begin{frame}{Blockchain mining as a competitive puzzle}
  ...
  \item[{\color{red}solution:}] \only<3->{reply with a \textbf{next} that satisfies the challenge}
  ...
  \item<7->[{\color{red}solution:}] reply with a \textbf{next}, with backwards pointer $\hash(\mathbf{block})$, obtained
...
\begin{frame}\frametitle{Signum's proof of space}
  ...
  \item<3->[{\color{red}solution:}] reply with a \textbf{next}, pointing back
  ...
  \visible<4->{The recomputation of the nonces on-the-fly is possible but must be too expensive to explore
  a sufficient number of nonces before the next block creation or otherwise a time/memory
  trade-off would make a PoW solution more rational}
...
\begin{frame}\frametitle{Proof of space: The general idea}
  ...
  \begin{description}
  \item[Ideal PoSp:] the computation of the solution can only be achieved by allocating
    a big data structure on disk (Ateniese et al., Dziembowski et al., Abusalah et al.)
    \medskip
  \item[Signum's PoSp:] the computation of the solution can also be achieved without
