...
\begin{frame}\frametitle{Signum's proof of space}
  ...
  \item<5->[{\color{red}competition:}] in case of more \textbf{next}, select the one with the smallest
    $\delay(\nonce,\challenge)$
  \end{description}
...
\begin{frame}{Point~1: The nonce data structure}
  ...
  \begin{center}
    ...
    \only<10>{$\scoop_i={\color{blue}h_{2i}},{\color{blue}h_{2n-2i-1}}$ depends from at least a ${\color{blue}h_j}$ with $j\ge n$}
    \only<11>{hash is the expensive shabal256}
    \only<12>{$\kappa$ allows one to modulate the cost of the computation}
  \end{center}
