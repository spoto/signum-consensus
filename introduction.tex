...
of the network. As shown in~\cite{GarayKL17}, this both stabilizes the block creation rate and
supports network consistency (all honest peers see the same chain, eventually).
...
no actual blockchain has ever been built using such theory: only SpaceMint~\cite{ParkKFGAP18} exists,
but it is not a real blockchain but only a minimal non-maintained prototype
of the theoretical consensus protocol. Namely, a real graph pebbling blockchain has
...
But also Permacoin has never been implemented: only a prototypical and minimal implementation
of its consensus algorithm exists, discontinued in~2014.

Our target of analysis has been Signum~\cite{Signum}, instead, since we wanted a fully-fledged implementation,
which gives practical relevance to our work. It is
actually deployed and runs continously since~2014. Nothing of that can be said about SpaceMint or
Permacoin, that only exist as minimal prototype implementations of only their consensus
algorithm, are not deployed blockchains and their code has been discontinued a decade ago.
Signum (previously Burstcoin) is not based on graph pebbling: instead,
...
but also by recomputing the plot on-the-fly, at each challenge, without allocating anything on disk
(proof of work). Because of that, Signum's mining is sometimes related as \emph{proof of capacity}.
However, Signum's proof of work version remains theoretical and there is no evidence that Signum has ever
been mined with proof of work. This is because plots use an expensive hashing algorithm (shabal256) and are very big.
Specialized hardware might change the situation in the future but, from its inception in 2014 to
the present day, Signum seems to have been mined with proof of space only.
...
Sec.~\ref{sec:conclusion} concludes. Proofs are reported as an appendix to the paper.
