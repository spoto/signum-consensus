...
This section formalizes the notions of nonce and plot and their
algorithmic construction. Namely, Signum requires miners to hold one or more plots
(sets of nonces)
...
\end{definition}
%
The following byte representation of natural numbers is used in~\cite{SignumPlotting}.
It is the standard representation in most computers nowadays.
%
\begin{definition}[$\nattobe$ and $\betonat$]
  The operators $\nattobe$ and $\betonat$ transform natural numbers
  into their big-endian byte representation, and vice versa. We recall that
  the big-endian representation of a natural number is its
  binary representation, split in bytes, with the most significant byte placed first.
\end{definition}
%
...
Their definitions are parametric \wrt a hashing function $h_\deadline$ used for their creation.
%
...
they silently introduce selection functions
for the tuple elements. For instance, if $\nonce\in\Nonces$, then $\nonce.p$
...
\end{definition}
%
Algorithm~\ref{alg:nonce_construction} constructs a nonce,
given its progressive number and a prolog.
It uses a constant $\kappa>0$ (Tab.~\ref{tab:notations})
to limit its computational cost, to avoid hashing very large chunks of data.
This algorithm derives a sequence of hashes (steps~\ref{step:nonce_construction:seed}
...
(\textit{ie.\ }a pair of consecutive hashes), the algorithm must be thoroughly executed.
%
\begin{alg}[$\nonce(p,\pi)$]\label{alg:nonce_construction}
  ...
  %
  \begin{enumerate}
  \item\label{step:nonce_construction:seed} Let $\seed=\pi\append\nattobe(p)$.
...
\end{alg}
%
Step~\ref{step:nonce_construction:modify} makes each $h_i$ dependent on the others, so that
one must compute them all in order to compute each of them.
This is strengthened by the use of the xor, that limits the risk of hash collisions.
Step~\ref{step:nonce_construction:swap} makes each scoop dependent on one $h_i$ with $i\ge\numberofscoops$,
so that smaller scoops are not easier to compute than the others. This avoids a potential
time/memory tradeoff (see Sec.~\ref{sec:prolog_structure_attacks}).
