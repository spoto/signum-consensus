...
%
This section uses the formalization of the previous sections to
extend the framework in~\cite{SignumPlotting} and
understand if nothing-at-stake attacks might work for Signum.
...
that the chain identifier of the blockchain is $b.\trunk.\delta.\pi.\chainidentifier$; and the other requires that
the signature of $b$ is verified with $b.\trunk.\delta.\pi.\publickeyofpeer$.
...
In~\cite{ParkKFGAP18}, Burstcoin was criticized because the validation of a deadline
(Def.~\ref{def:deadline_validity}) requires to run
Alg.~\ref{alg:nonce_construction} that, they say,
...
with a minimal energy cost. Moreover, it is largely dominated
by that for verifying the transactions in a block, in particular
for Signum that allows smart contracts.
Therefore, this time for deadline validation seems largely acceptable to us.
...
Considering $\kappa$ and the specific values and hashing
used in~\cite{SignumPlotting} (rightmost column of Tab.~\ref{tab:notations}),
...
hashes $33816776$ bytes, around $8$ times less than what was reported in~\cite{ParkKFGAP18}.
...
Consequently, it cannot be grinded.
%
\begin{proposition}\label{prop:no_block_grinding}
Signum is protected against block-grinding attacks.
\end{proposition}
\begin{proof}
  The next challenge of a block depends on the trunk of the block only (Def.~\ref{def:next_challenge_from_trunk}).
  In particular, it does not depend on any other component of the block, such as the set of
  transactions that the miner decides to include in it.\qed
\end{proof}
%
%We sympathize with the authors of~\cite{ParkKFGAP18}:
Without this formalization, it was impossible to exclude that attack.

We furthermore observe that Signum has some protection
against challenge-grinding attacks.
The challenge for the a block is actually uniquely determined by the trunk of its
previous block
(rule~\ref{prop:consensus:answer} of Def.~\ref{def:blockchain}
...
of Alg.~\ref{alg:block_mining} (\textit{ie.}, not the minimal one
of Def.~\ref{def:deadline_from_plot}),
hoping that such a sacrifice will lead to subsequent challenges for which it
can find very good deadlines.
But Prop.~\ref{prop:sequence_of_challenges} shows that that
choice does not pay off: deadlines do not
affect the sequence of challenges, only the plots do.
%
\begin{proposition}\label{prop:sequence_of_challenges}
  ...
  \end{align*}
  %
  for suitable $\nonce_i,\nonce_i'\in\plot.\nonces$, for every $1\le i\le n$. Then it holds
  $%\[
...
\begin{proposition}\label{prop:no_challenge_grinding}
  Signum is largely protected against challenge-grinding attacks.
\end{proposition}
\begin{proof}
  By Prop.~\ref{prop:sequence_of_challenges}, the
  sequence of challenges depends only on
  ...
  would require a peer to use more plots, whose prologs have different keys, all in control of the peer,
  ...
  limits the benefits of grinding.\qed
\end{proof}
%
For additional protection against challenge-grinding attacks, it is always possible to use
...
reconstruct the smaller scoops on demand. The resulting (mining power)/(space used) ratio was proportionally higher
...
With that extra step, each scoop contains a $h_i$ with $i\ge\numberofscoops$, so that
at least half of all $h_i$'s must be kept in memory in order to compute any scoop.
